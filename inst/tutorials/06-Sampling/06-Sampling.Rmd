...
First, examining the population distribution:
...
1. Takes multiple samples of a specified size
...
Try running this code multiple times with different sample sizes to see how the distribution changes:

```{r sampling-distribution-setup}
# Function to simulate sampling distribution
# n_samples: number of samples to take
# sample_size: size of each sample
simulate_samples <- function(n_samples, sample_size) {
...
# Get sample means
samples <- simulate_samples(n_samples, sample_size)
...
The code below compares sampling distributions for different sample sizes. Run it multiple times and try modifying the sample sizes to see how the distributions change:

```{r clt-setup}
# Function to get sample means
get_sample_means <- function(data, n_samples, sample_size) {
...
means_small <- get_sample_means(NHANES_cleanAlc, 1000, small_n)
means_large <- get_sample_means(NHANES_cleanAlc, 1000, large_n)
...
```{r ci-simulation, exercise=TRUE, exercise.setup="ci-setup"}
...
```{r implement-design, exercise=TRUE, exercise.setup="ci-setup"}
