...
This virtual environment contains a folder named data placed at the root. This means that the filepath to that folder is `./data`, within that data folder are several files that we will learn how to load in memory in order to do some data analysis.


### Loading a csv file

First, in the `./data` folder, you will find a csv (comma-separated value) file that you can read using the `read.csv()` function, the file is named 'bsa2021_healthcare_equalities_recoded.csv'.
...
Tab separated files are very common. Like CSV files they are simple text files. But instead of a comma, they use tabulation as a separator.
